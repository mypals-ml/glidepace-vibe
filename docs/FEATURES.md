...
*   **Task Reordering & Positioned Creation:** Tasks keep the default order returned by GitHub Projects and can be reordered from the sidebar using drag-and-drop. The app persists order through GitHub Projects V2's native item position mutation, so large moves only update the moved item position instead of rewriting custom order fields. Filtered task lists can also be reordered relative to visible neighbors. Long-pressing a task on mobile, or opening the desktop row "more" menu, lets users create a new task directly above or below the selected task.
