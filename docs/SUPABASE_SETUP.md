...
Add **all four** variables to your `.env.local` file. Locally, everything runs on your machine — Vite serves the frontend and `vite-plugin-vercel-mock.ts` runs the API handlers in the same process:
...
> [!NOTE]
> The values are **identical** between `.env.local` and Vercel — it's the same Supabase project. You just configure them in two different places because local dev reads from a file while production reads from the Vercel dashboard.
